...
def DefStrength( source : BinaryQuadraticModel, embedding : EmbeddedStructure, multiplier: float):
    global cached
...
current_solution : dict[int,int] = {}

sampleset = SASampler.sample(model, num_reads = 10000)


for i in model.variables:
    j = sampleset.variables.index(i)
    current_solution[i] = sampleset.record[0].sample[j]

print('Simulated annealing found solution:')
print(current_solution)

for (u,v) in model.quadratic.keys():
    w = model.get_quadratic(u,v)
    w *= current_solution[u]
    w *= current_solution[v]
...
for i in range(len(cs_list)):
    mult = cs_list[i]
    finalStrength = partial(DefStrength, multiplier = mult)
