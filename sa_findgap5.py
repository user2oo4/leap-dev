...
def get_d(j: float, delta: float) -> float:
    f1 = compute(j + delta / 2, FCSampler, DEF_NS)['break'] * DEF_NS * n
    f2 = compute(j - delta / 2, FCSampler, DEF_NS)['break'] * DEF_NS * n
...
f = open(f'findgap5_runtime.csv', mode='a')
print(f'Run time: {(time.time() - start_time)} seconds')
f.write(f'{iname},{(time.time() - start_time)}\n')
f.close()

f = open(f'findgap5_results_p/{iname}.csv', 'w', encoding='utf-8')
